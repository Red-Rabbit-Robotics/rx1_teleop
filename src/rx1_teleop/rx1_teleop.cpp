      ...
      priv_nh_(priv_nh)
{
    ...
    for (int i = 0; i < ARM_SERVO_NUM_; ++i)
    {
        ...
        right_arm_joint_state_msg_.name[i] = joint_name[i];
    }
...
void Rx1Teleop::update()
{
    ...
    for (int i = 0; i < 7; i ++)
    {
        ...
        //    ROS_INFO("servo %d pos: %d", i, sts_servo_.ReadPos(sts_servo_ids_[i]));
            right_arm_joint_state_msg_.position[i] = sts_servo_dirs_[i] * (static_cast<float>(sts_servo_.ReadPos(sts_servo_ids_[i])) - 2048.0) / 2048.0 * 3.14;
        //}
    }
